    ...
    try:
        ...
        for item in items:
            ...
            stremio_metas.append({
                "id": f"kitsu:{a_id}",
                "type": "movie" if kitsu_type == "movie" else "series",
...
async def addon_meta(user_id: str, catalog_type: str, stremio_id: str):
    ...
    try:
        ...
        # Wichtig für Torrentio/Debrid: Diese Felder lassen externe Addons suchen
        if imdb_id: meta["imdb_id"] = imdb_id
        if mal_id: meta["mal_id"] = mal_id

        # 2. Episoden-Array mit echten Titeln und Daten
        if meta["type"] == "series":
            ...
            # Auffüllen falls Kitsu-Liste unvollständig
